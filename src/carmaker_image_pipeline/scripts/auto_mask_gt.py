...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Lower R threshold for yellow landmark detection (default: 200).",
    )
    parser.add_argument(
        "--yellow-g-min",
        type=int,
        default=100,
        help="Lower G threshold for yellow landmark detection (default: 100).",
    )
    parser.add_argument(
        "--yellow-b-max",
        type=int,
        default=80,
        help="Upper B threshold for yellow landmark detection (default: 80).",
    )
    parser.add_argument(
        "--recursive",
        action="store_true",
...
def classify_post_processed_image(
    ...
    class_map = np.zeros(image_bgr.shape[:2], dtype=np.uint8)

    lane_mask = np.all(image_bgr <= lane_threshold, axis=2)
    ...
    if car_mask is not None:
        ...
        yellow_mask[masked_region] = False

    class_map[lane_mask] = CLASS_LANE_BLACK
    class_map[yellow_mask] = CLASS_LANDMARK_YELLOW
    return class_map
...
def main():
    ...
    for image_path in images:
        ...
        class_map = classify_post_processed_image(
            ...
            yellow_r_min=args.yellow_r_min,
            yellow_g_min=args.yellow_g_min,
            yellow_b_max=args.yellow_b_max,
        )
