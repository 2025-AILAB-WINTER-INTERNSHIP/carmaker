...
vehicle:
  ...
  dynamics_pose_reference: "rear_bumper"
  steering_ratio: 7.0
  max_steer_command: 3.5
  steering_command_sign: 1.0
  steering_control_point: "rear_axle"
```

Stanley는 타이어 조향각을 계산한다. CarMaker `control_signal/steerangle`이 steering wheel command 범위를 기대하는 프로젝트라면 `steering_ratio`와 `max_steer_command`를 teleop 입력 범위와 맞춰야 한다. 현재 기본값은 `carmaker_teleop` launch의 `max_steer: 3.5`와 맞춘다.
...
`steering_control_point`는 Stanley 오차 계산 기준점이다. `/planning/trajectory`는 RViz 확인을 위해 rear bumper 기준으로 publish되지만, planning 내부 경로와 차량 기구학은 rear axle 기준이다. 주차처럼 곡률이 큰 경로에서는 `rear_axle` 기준으로 cte/heading error를 계산하는 편이 안정적이다.
