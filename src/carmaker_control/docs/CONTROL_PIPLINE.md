...
planning의 trajectory publisher는 latched publisher이므로, control node를 planning 이후에 켜도 마지막으로 publish된 trajectory를 받을 수 있다. 다만 local planner가 final hold 이후 active trajectory publish를 멈추면 control은 `active_trajectory_timeout` 뒤 neutral stop으로 돌아간다.
...
마지막으로 global planner는 `/planning/global/trajectory`를 publish하고, local planner의
`TrajectoryStateMachine`이 현재 따라야 하는 single active segment를 `/planning/trajectory`로
publish한다. direction은 `TrajectoryPoint.direction` 필드로 명시하고,
`longitudinal_velocity`도 기존 물리량 해석을 위해 direction 부호를 반영한 signed velocity로 유지한다.
...
control은 `TrajectoryPoint.direction`을 기어 방향 계약으로 사용한다.

| `direction` | control 해석 |
| ----------- | ------------ |
| `1`         | 전진 gear    |
| `-1`        | 후진 gear    |
...
planner 내부에서는 direction이 바뀌는 cusp 기준으로 global path를 segment로 나누고,
control에는 현재 따라야 할 single active trajectory만 보낸다. control은 더 이상
`splitTrajectory()`를 갖지 않고, `TrajectoryPoint.direction`을 gear 결정에 사용한다.

정지점이나 기어 전환점에서는 planner가 같은 direction의 all-zero velocity trajectory를
publish한다. control은 이를 stop/presteer 명령으로 보고 해당 direction gear를 유지한 채
brake와 steering command를 낸다.

control이 전체 global trajectory가 아니라 active trajectory 안에서만 nearest point를 찾는 이유는,
전진/후진 경로가 가까이 붙어 있을 때 잘못된 segment를 따라가는 문제를 planner 단계에서 이미
해결했기 때문이다.
...
`active_trajectory_timeout` 안에 새 active trajectory가 없으면 경로가 오래되었다고 보고 neutral stop 명령을 낸다.

### 전진/후진 direction 판단

control은 `TrajectoryPoint.direction`을 필수 계약으로 사용한다. direction이 없거나 한 active trajectory 안에서 섞인 메시지는 무시한다.
...
`publish`를 `false`로 두면 control debug 토픽을 끌 수 있다. `path_publish_period`는 `/control/debug/active_path` publish 주기이며, 0 이하로 두면 매 control tick마다 publish한다.
...
- RViz에서 `/control/debug/current_pose`가 `/control/debug/active_path` 위에 놓이는지
...
- `/control/debug/current_pose`와 `/control/debug/active_path`의 기준점이 같은지
