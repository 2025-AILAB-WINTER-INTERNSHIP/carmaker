...
사용법:
    ...
    CameraBundle 결과 비교:
    python compare_bags_sync.py \\
        --fp32-bag fp32_bundle_result.bag \\
        --fp16-bag fp16_bundle_result.bag \\
...
def _topic_type(bag_path: str, topic: str) -> str:
    """Bag 안의 topic message type을 반환합니다."""
    with rosbag.Bag(bag_path) as bag:
...
def _image_entries(msg: Any) -> list[tuple[str, Any]]:
    """Image 또는 CameraBundle 메시지를 비교 가능한 이미지 목록으로 변환합니다.
    ...
    if hasattr(msg, "images"):
        ...
        for idx, image_msg in enumerate(msg.images):
            label = names[idx] if idx < len(names) and names[idx] else f"image_{idx}"
            entries.append((label, image_msg))
        return entries
...
def compare_synced_bags(
    ...
    Notes:
        ...
        (overall_accuracy)을 사용합니다. matching_images는 참고 지표입니다.
    """
    fp32_topic_type = _topic_type(fp32_bag_path, topic)
    fp16_topic_type = _topic_type(fp16_bag_path, topic)
    ...
    print(
        ...
        f"  - 토픽: {topic}\n"
        f"  - 메시지 타입(FP32/FP16): {fp32_topic_type or 'N/A'} / {fp16_topic_type or 'N/A'}\n"
        f"  - 통과 기준: {threshold:.2f}% (누적 픽셀 일치율 기준)"
    )

    if not fp32_topic_type or not fp16_topic_type:
        print("🚨 오류: 지정한 topic을 두 Bag 중 하나에서 찾을 수 없습니다.")
        return False

    if fp32_topic_type != fp16_topic_type:
        print(
            "🚨 오류: 두 Bag의 topic message type이 다릅니다.\n"
            f"   FP32: {fp32_topic_type}\n"
            f"   FP16: {fp16_topic_type}"
        )
        return False
    ...
    for i, ts in enumerate(common_timestamps):
        ...
        entries_fp32 = _image_entries(msg_fp32)
        entries_fp16 = _image_entries(msg_fp16)

        if len(entries_fp32) != len(entries_fp16):
            print(
                f"  -> [프레임 {i}] 이미지 개수 불일치로 건너뜀 "
                f"(FP32:{len(entries_fp32)} vs FP16:{len(entries_fp16)})"
            )
            skipped_images += max(len(entries_fp32), len(entries_fp16))
            structural_errors += 1
            continue

        compared_bundles += 1

        for image_idx, ((label_fp32, image_fp32), (label_fp16, image_fp16)) in enumerate(
            zip(entries_fp32, entries_fp16)
        ):
            label = label_fp32
            if label_fp32 != label_fp16:
                print(
                    f"  -> [프레임 {i}, 이미지 {image_idx}] 이름 불일치 "
                    f"(FP32:{label_fp32} vs FP16:{label_fp16})"
                )
                label = f"{label_fp32}_vs_{label_fp16}"
                structural_errors += 1

            img_fp32 = bridge.imgmsg_to_cv2(image_fp32, desired_encoding="passthrough")
            img_fp16 = bridge.imgmsg_to_cv2(image_fp16, desired_encoding="passthrough")

            if img_fp32.shape != img_fp16.shape:
                print(
                    f"  -> [프레임 {i}, 이미지 {image_idx}:{label}] 해상도 불일치로 건너뜀 "
            ...
            accuracy = (matching_pixels / image_pixels) * 100.0
            ...
            if accuracy >= threshold:
                ...
                diff_path = output_dir / f"diff_ts_{ts}_idx_{image_idx}_{safe_label}.png"
                ...
                print(
                    f"  -> [주의] TS:{ts}, 이미지 {image_idx}:{label} "
                    f"일치율 {accuracy:.4f}% (오차 이미지 저장됨)"
                )
    ...
    if total_pixels == 0:
        print(
            "\n🚨 비교 가능한 이미지가 없습니다 (모든 이미지가 구조/해상도 불일치로 건너뜀).\n"
    ...
    print(f"  공통 프레임      : {len(common_timestamps)}")
    print(f"  비교된 bundle    : {compared_bundles}")
    print(f"  통과 이미지      : {matching_images} / {compared_images}  "
          f"(실패: {failed_images}, 건너뜀: {skipped_images})  ← 참고 지표")
    print(f"  구조 오류        : {structural_errors}")
    print(
        f"\n  [참고] 통과/실패 판정은 누적 픽셀 기준 전체 일치율(overall_accuracy)을\n"
        f"         사용합니다. 통과 이미지 수는 별도 참고 지표입니다."
    )

    if overall_accuracy >= threshold and structural_errors == 0:
        print(
            f"\n🚀 [결론] 검증 통과! "
